//Sydney Opera House as default location
var currentLatitude = -33.8567844; 
var currentLongitude = 151.215256; 
var selectedLatitude = -33.8567844; 
var selectedLongitude = 151.2152967; 
...
function fetchYelpData() {
    $.ajax({
        ...
        for (i = 0; i < data.businesses.length; i++) {
            ...
            var restModalReviews = $('<p class="review">' + data.businesses[i].review_count + ' review' + pluralReviews + ' for average of ' + data.businesses[i].rating + ' stars</p>')
            ...
            var priceRating = $('<p class="price">' + data.businesses[i].price + '</p>');
            var streetAddress = $('<address class="address"><br>' + data.businesses[i].location.address1 + '<br>' + data.businesses[i].location.city + ' ' + data.businesses[i].location.state + ' ' + data.businesses[i].location.zip_code + '</address>');
            var restaurantPhone = $('<p class="phone">Phone: ' + data.businesses[i].display_phone + '</p>');
